        ...
        <div className="min-h-screen bg-[#0A1420] py-10 px-6">
            <div className="max-w-5xl mx-auto space-y-6">
                ...
                <div className="flex items-center justify-between mb-8">
                    <div>
                        <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3"><Rocket size={28} className="text-indigo-400" /> Pre-boarding Dashboard</h1>
                        <p className="text-[#8899AA]">Welcome aboard! Let's get everything ready before your Day 1 on Dec 01, 2025.</p>
                    </div>

                    <div className="bg-[#131B2B] border border-[#2A3A4A] rounded-xl p-4 flex gap-6 text-center">
                        <div>
                        ...
                        <div className="w-px bg-[#2A3A4A]"></div>
                        <div>
                ...
                <div className="grid md:grid-cols-3 gap-8 text-black">
                    ...
                    <div className="md:col-span-2 space-y-6">
                        ...
                        <div className="space-y-4">
                            {TASKS.map((t, i) => {
                                const Icon = t.icon;
                                return (
                                    <div key={i} className={`p-5 rounded-2xl border transition-all ${t.status === 'done' ? 'bg-[#060D1A] border-[#1A2A3A] opacity-70' : t.status === 'active' ? 'bg-[#131B2B] border-indigo-500/50 shadow-[0_0_20px_rgba(79,70,229,0.1)] border-l-4 border-l-indigo-500' : 'bg-[#131B2B] border-[#2A3A4A] border-l-4 border-l-transparent text-gray-400'}`}>
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-start gap-4">
                                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${t.status === 'done' ? 'bg-emerald-500/10 text-emerald-500' : `bg-[#0A1420] border border-[#2A3A4A] ${t.color}`}`}>
                                                    {t.status === 'done' ? <CheckCircle2 size={24} /> : <Icon size={20} />}
                                                </div>
                                                <div>
                                                    <h3 className={`font-bold mb-1 ${t.status === 'done' || t.status === 'pending' ? 'text-[#AABBCC]' : 'text-white'}`}>{t.label}</h3>
                                                    <p className="text-[#556677] text-sm">{t.desc}</p>
                                                </div>
                                            </div>

                                            {t.status === 'active' && (
                                                <button className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold px-5 py-2 rounded-lg text-sm transition-colors flex items-center gap-1">
                    ...
                    <div className="space-y-6">
                        <div className="bg-gradient-to-br from-indigo-900/40 to-purple-900/40 border border-indigo-500/30 rounded-2xl p-6 relative overflow-hidden text-center text-white">
                            ...
                            <button className="w-full bg-[#0A1420] hover:bg-[#131B2B] text-white font-bold py-2 rounded-xl text-sm transition-colors border border-[#2A3A4A] relative z-10">
                                Say Hello
                            </button>
                        </div>
