...
import {
    FileText, CheckCircle2, Circle, ShieldAlert, Download, ChevronRight,
} from "lucide-react";
...
export default function PolicyAcknowledgment() {
    ...
    const allMandatoryAgreed = POLICIES.filter((p) => p.required).every((p) =>
        agreedPolicies.includes(p.id)
    );

    return (
        <Page
            title="Policy Compliance"
            subtitle="Please review and acknowledge all mandatory company policies (3/4 Remaining)."
            breadcrumbs={[
                { label: "Home", href: "/" },
                { label: "Onboarding", href: "/onboarding/dashboard" },
                { label: "Policy Acknowledgment", href: "/onboarding/policy" },
            ]}
        >
            <div className="flex flex-col md:flex-row gap-6" style={{ minHeight: "calc(100vh - 260px)" }}>
                {/* Policy List Sidebar */}
                <div className={`w-full md:w-[450px] shrink-0 bg-[#0F1C2E] border border-[#1A2A3A] rounded-2xl flex flex-col ${viewingPolicy ? "hidden md:flex" : "flex"}`}>
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full text-left p-4 rounded-xl border cursor-pointer transition-all ${
                                        viewingPolicy === policy.id
                                            ? "bg-[#1A2A3A] border-[#33E6FF]"
                                            : agreed
                                            ? "bg-[#0A1420]/50 border-[#1A2A3A] opacity-60 hover:opacity-100"
                                            : "bg-[#0A1420] border-[#1A2A3A] hover:border-[#2A3A4A]"
                                    }`}
                                    aria-pressed={viewingPolicy === policy.id}
                                >
                    ...
                    <div className="p-6 border-t border-[#1A2A3A]">
                        <Button
                            disabled={!allMandatoryAgreed}
                            className="w-full justify-center"
                        >
                            Complete Compliance <ChevronRight size={18} aria-hidden="true" />
                        </Button>
                    </div>
                </div>

                {/* Policy Viewer Main Area */}
                <div className={`flex-1 flex flex-col bg-[#0F1C2E] border border-[#1A2A3A] rounded-2xl overflow-hidden ${!viewingPolicy ? "hidden md:flex" : "flex"}`}>
                    {viewingPolicy ? (
                        <>
                            {/* Viewer Header */}
                            <div className="h-16 shrink-0 border-b border-[#1A2A3A] bg-[#0F1C2E] flex items-center justify-between px-6">
                                ...
                                <div className="flex items-center gap-2">
                                    <button
                                        type="button"
                                        className="p-2 text-[#8899AA] hover:text-white transition-colors"
                                    ...
                                    <button
                                        type="button"
                                        className="p-2 text-[#8899AA] hover:text-white transition-colors md:hidden"
                            ...
                            <div className="flex-1 overflow-y-auto p-8 bg-[#0A1420] flex justify-center">
                                <div className="w-full max-w-3xl min-h-[800px] bg-white rounded-lg shadow-2xl pointer-events-none select-none p-12 relative flex flex-col pt-32 overflow-hidden border border-gray-200">
                                    ...
                                    <div className="absolute right-12 top-24 opacity-10">
                                        <ShieldAlert size={120} className="text-black" aria-hidden="true" />
                                    </div>

                                    <h1 className="text-3xl font-black text-gray-900 mb-6 border-b-4 border-blue-900 pb-4 inline-block tracking-tight">
                            ...
                            <div className="shrink-0 bg-[#0F1C2E] border-t border-[#1A2A3A] p-6 shadow-[0_-10px_30px_rgba(0,0,0,0.5)] z-20">
                                <label
                                    htmlFor={`policy-agree-${viewingPolicy}`}
                                    className={`flex items-start gap-4 p-4 rounded-xl border cursor-pointer transition-colors ${
                                        agreedPolicies.includes(viewingPolicy)
                                            ? "bg-[#00E5A0]/5 border-[#00E5A0]/30"
                                            : "bg-[#1A2A3A] border-[#2A3A4A] hover:border-[#445566]"
                                    }`}
                                >
                                    <div className="relative mt-1">
                                        <input
                                            id={`policy-agree-${viewingPolicy}`}
                                            type="checkbox"
                                            className="sr-only"
                                        ...
                                        <div
                                            className={`w-6 h-6 rounded border-2 flex items-center justify-center transition-colors ${
                                                agreedPolicies.includes(viewingPolicy)
                                                    ...
                                                    : "border-[#445566] bg-[#0A1420]"
                                            }`}
                                        >
                                            {agreedPolicies.includes(viewingPolicy) && (
                                                <CheckCircle2 size={16} className="text-[#0A1420]" aria-hidden="true" />
                                            )}
                                        </div>
                                    </div>
                                    <div className="flex-1">
                                        ...
                                        <p className="text-sm text-[#8899AA] mt-1">
                                            By checking this box, I electronically sign and acknowledge my compliance with this document.
                                        </p>
                                    </div>
                                </label>
                            </div>
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-[#445566] p-8 text-center">
                            <ShieldAlert size={64} className="mb-6 opacity-20" aria-hidden="true" />
                            <h2 className="text-xl font-medium text-white mb-2">Select a policy to review</h2>
                            <p className="text-sm max-w-sm">
                                You must read and acknowledge all mandatory policies listed in the sidebar before completing your pre-boarding.
                            </p>
                        </div>
                    )}
                </div>
