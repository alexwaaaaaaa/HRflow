...
const TABS = ["Radar View", "Bar Comparison", "Qualitative Themes"] as const;
...
export default function FeedbackReportPage() {
    ...
    return (
        <Page
            ...
            subtitle="Priya Mehta · Mid-Year 2025 · 9 Reviewers"
            breadcrumbs={[
        ...
        >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {SUMMARY.map((s) => (
                    <Card key={s.label} padding="md">
                        <p className="text-xs text-[#8899AA] mb-2">{s.label}</p>
                        <p className="text-2xl font-bold mb-1" style={{ color: s.color }}>
                            {s.value}
                        </p>
                        <span
                            className={`flex items-center gap-1 text-xs font-medium ${
                                s.up ? "text-[#00E5A0]" : "text-[#FF4444]"
                            }`}
                        >
                            {s.up ? (
            ...
            {/* Tabs */}
            <div
                className="flex border-b border-[#1A2A3A]"
                role="tablist"
                ...
                {tab === 0 && (
                    <Card padding="lg">
                        <h2 className="text-sm font-semibold text-white mb-4">
                            Competency Radar — Self vs Peers vs Manager
                        </h2>
                        <div className="h-72">
                            ...
                            <li key={t.theme}>
                                <Card padding="md">
                                    <div className="flex items-center gap-2 mb-3">
                                        <Badge variant={t.type === "strength" ? "success" : "warning"}>
                                    ...
                                    <ul role="list" className="space-y-2">
                                        {t.quotes.map((q, i) => (
                                            <li key={i} className="flex items-start gap-2">
                                                <span className="text-[#445566] mt-0.5" aria-hidden="true">
                                                    &ldquo;
                                                </span>
                                                <p className="text-sm text-[#CCDDEE] italic">{q}&rdquo;</p>
                                            </li>
                                        ))}
                                    </ul>
                                </Card>
